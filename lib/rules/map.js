  ...
  create: function(context) {
    ...
    return {
      Identifier(node) {
        ...
        if (hasAncestorFunction(context.getAncestors())) return;
        ...
        if (
          ...
          const longTemp = `Array.isArray(${firstArgText}) ? ${firstArgText}.map(${secondArgText}) : _.map(${firstArgText}, ${secondArgText})`;
        ...
        function hasAncestorFunction(ancestors) {
          const fnNodeTypes = [
          ...
          return ancestors.some(a => {
            return fnNodeTypes.includes(a.type);
          });
        }
      }
    };
